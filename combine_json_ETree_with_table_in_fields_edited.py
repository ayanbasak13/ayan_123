...
def get_field_xml(field,xmd,print_flag=False):
    ...
    if field['type'] == 'table':
        ...
        elif field['type'] == "group_frame" or field['type'] == "Group-Frame":
            for val in label_and_value:
                if val['name'] == 'Text Value':
                    value_word = val['key']
                    break
        ...
        xmd.set('value',value_word)
        xmd.set('tag',field['tag'])
    ...
    if len(page_structure)<1 :
        ...
        for textline in page_structure:
            ...
            if is_finalised_page_structures:
                ...
                ids=[]
                for p in popped:
                    ids.append(fields_list[p]['_id'])
                print('@'*10,ids,textline,len(fields_list))
                if str(textline)=='Mail To:':
                    print('got it')
